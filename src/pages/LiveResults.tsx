...
import { liveResults, overallStandings } from "@/data/mock-data";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { Badge } from "@/components/ui/badge";
import { Clock, Flag } from "lucide-react";

const LiveResults = () => {
  const [selectedRally, setSelectedRally] = useState<string>("crete-rally-2024");
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      const results = liveResults.filter(result => result.rallyId === selectedRally);
      setRallyResults(results);
      
      const standings = overallStandings.find(standing => standing.rallyId === selectedRally);
                      ...
                      <TabsContent value="stages" className="p-4">
                        {rallyResults.length > 0 ? (
                          rallyResults.map((result) => (
                            <div key={result.stageId} className="mb-6 last:mb-0">
